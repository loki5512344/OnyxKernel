...
impl DirNode {
    pub fn root() -> Self {
        DirNode { ino: 1, parent_ino: 1, entries: Vec::new() }
    }
}
...
pub fn add_dir(dirs: &mut Vec<DirNode>, path: &str, next_ino: &mut u32) {
    ...
    *next_ino += 1;
    ...
    dirs.push(DirNode { ino, parent_ino: dirs[parent].ino, entries: Vec::new() });
    eprintln!("  dir {} (ino={})", path, ino);
}

pub fn add_file(
    dirs: &mut Vec<DirNode>, files: &mut Vec<Entry>,
    host: &str, fs_path: &str, next_ino: &mut u32,
) {
    let data = std::fs::read(host).unwrap_or_else(|e| {
        eprintln!("read {}: {}", host, e); std::process::exit(1);
    });
    ...
    *next_ino += 1;
    ...
    eprintln!("  {} -> {} (ino={}, {} bytes)", host, fs_path, ino, files.last().unwrap().data.len());
}

pub fn add_dir_recursive(
    dirs: &mut Vec<DirNode>, files: &mut Vec<Entry>,
    host_dir: &str, fs_prefix: &str, next_ino: &mut u32,
) {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in std::fs::read_dir(&dir).unwrap_or_else(|e| { eprintln!("read_dir {}: {}", dir.display(), e); std::process::exit(1); }) {
            let entry = entry.unwrap_or_else(|e| { eprintln!("read_dir entry: {}", e); std::process::exit(1); });
            ...
            if let Ok(relative) = entry_path.strip_prefix(host_root) {
                ...
                if entry.file_type().map(|t| t.is_dir()).unwrap_or(false) {
                    ...
                    add_file(dirs, files, entry_path.to_str().unwrap(), &fs_path, next_ino);
                }
            }
        }
    }
...
pub fn ensure_parent_dirs(dirs: &mut Vec<DirNode>, path: &str, next_ino: &mut u32) {
    ...
    for comp in path.split('/') {
        if comp.is_empty() || comp == basename(path) { continue; }
        cur.push('/');
        cur.push_str(comp);
        ...
        if !exists { add_dir(dirs, &cur, next_ino); }
    }
}

fn find_parent_dir(dirs: &[DirNode], path: &str) -> usize {
    let path = path.trim_start_matches('/');
    if !path.contains('/') { return 0; }
                ...
                for (name, _ino, _is_dir) in &dirs[cur_idx].entries {
                    if name == comp {
                        for (j, dd) in dirs.iter().enumerate() {
                            if dd.ino == *_ino { cur_idx = j; found = true; break; }
                        }
                        if found { break; }
                    }
                }
...
fn basename(path: &str) -> &str {
    let path = path.trim_start_matches('/');
    match path.rfind('/') { Some(i) => &path[i + 1..], None => path }
}
