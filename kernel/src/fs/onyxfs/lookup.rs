...
/// Walk path resolving symlinks as they are encountered. Recursion is limited
/// to 8 hops to detect loops. For relative symlink targets, the parent
/// directory path is reconstructed from the original `path` up to (but not
/// including) the symlink component.
